...
| 版本號類型 | 目前值 | 定義 | 遞增時機 |
| :---- | :---- | :---- | :---- |
| **文件版本 (Document Version)** | **v1.3** | 本規格書自身的修訂版次。描述「我們打算做什麼」。 | 規格內容有實質變更時遞增（如新增工具、修改 Schema、調整架構）。 |
...
| **v1.3** | **2026-09-04** | **依真實素材實測修正分類器訊號排序**。見下方變更明細。 |
...
7. **【新增】** §5.6.3.5 降低對自動分類的依賴——自動分類降級為建議與預設值，CLI 須提供覆寫。
...
> **靜態黑邊剝除（§5.6.7）必須在分類之前執行。**

實測發現 letterbox／pillarbox 黑邊會在畫面邊界產生強垂直邊緣，使軸向邊緣佔比虛高至 0.73（實際為自然實拍）。順序顛倒會系統性汙染分類結果。
...
| 文字區塊密度素材層級最佳門檻（$k=8$） | **4.9**（召回率 0.91／特異度 0.95） |
...
classify_text_density_threshold  = 4.9    # 主訊號，暫定：召回 0.91 / 特異度 0.95
...
| **R-13** | **校準語料不足**（v1.2 新增，v1.3 更新） | **高** | §5.6.3 的門檻仍未以多來源語料校準。**此風險已部分實現**：v1.2 依合成樣本推定的主訊號與門檻，經單一真實素材驗證即遭推翻（分類準確率 0.49，低於基準線 0.70）——證明未經真實語料的門檻不僅不準，還可能比不分類更糟。 | ① 主訊號已改為文字區塊密度，門檻 4.9 為單一來源暫定值，仍須多來源確認；輔助訊號門檻維持 `-1`（未校準）；② **實作在主訊號未校準時必須拒絕啟動自動分類**，改要求使用者顯式指定素材類型，而非用猜測值靜默運行；③ §5.6.3.5 已把自動分類降級為建議值，CLI 須提供 `--content-class` 覆寫，降低對分類器的依賴；④ 語料收集與門檻校準列為 **M1 退出條件**；⑤ `scripts/calibrate_classifier.py` 自動化校準流程，語料擴充後可重跑。 |
...
- [x] 首輪訊號驗證：以 1 支真實影片（120 取樣點／48 標註）推翻 v1.2 訊號排序，確立文字區塊密度為主訊號（§5.6.3.4）。
...
| **A-8** | **校準語料規模與涵蓋面**：已用 1 支真實影片完成首輪驗證（§5.6.3.4），推翻了 v1.2 的訊號排序。仍需多來源語料確認文字區塊密度門檻。**特別需要目標情境的素材**——首輪語料的 `screen` 樣本多為紀錄片式截圖插入，而非使用者自錄的完整螢幕錄影。建議涵蓋：深色／淺色 IDE 主題、不同 UI 縮放比、中文／英文介面、終端機、簡報錄製。 | 直接決定 R-13 的殘餘風險 | M1 開始前 |
