...
| 版本號類型 | 目前值 | 定義 | 遞增時機 |
| :---- | :---- | :---- | :---- |
| **文件版本 (Document Version)** | **v1.7** | 本規格書自身的修訂版次。描述「我們打算做什麼」。 | 規格內容有實質變更時遞增（如新增工具、修改 Schema、調整架構）。 |
...
| **v1.7** | **2026-09-04** | **Spike 1：首次實際執行濾鏡鏈與管線。7/7 通過，但記憶體模型低估 1.85 倍。** 見下方變更明細。 |
...
librosa 無原生 downbeat 偵測能力，v1.0 只能以 `infer_4_4` 推定（R-07）。但重拍通常正是起音能量較高的拍點，故能量加權可在不引入 `madmom`（有非商業授權限制，見 §11.2）的前提下，取得部分重拍對齊的效果。

**證據等級**：加權公式的形式為 **C（外部建議，未實測）**；穩健正規化與索引對齊要求為 **A（librosa 官方文件明載回傳值未正規化）**。加權效果須於 M2 以實際音訊驗證。
...
**M1 目前進度**：濾鏡鏈驗證與分類器校準已完成；**剩餘阻擋項為音訊模組**（librosa 安裝與 §5.2 / §5.2.1 節拍分析，即 Spike 2）。
