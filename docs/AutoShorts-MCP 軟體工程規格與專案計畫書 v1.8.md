...
| :---- | :---- |
| **文件版本** | v1.8 |
| **專案版本** | v0.1 (Pre-Alpha / 規格定版；M1 進行中) |
...
| 版本號類型 | 目前值 | 定義 | 遞增時機 |
| :---- | :---- | :---- | :---- |
| **文件版本 (Document Version)** | **v1.8** | 本規格書自身的修訂版次。描述「我們打算做什麼」。 | 規格內容有實質變更時遞增（如新增工具、修改 Schema、調整架構）。 |
...
| v1.4 | 2026-09-04 | 跨 20 支獨立來源實測，修正 v1.3 的兩項過度宣稱。 |
| v1.5 | 2026-09-04 | 外部架構建議評估與查證後採納。 |
| v1.6 | 2026-09-04 | 取得 screen 類語料，找到癱瘓分類器四個版本的根因。 |
| v1.7 | 2026-09-04 | Spike 1：首次實際執行濾鏡鏈與管線，7/7 通過。 |
| **v1.8** | **2026-09-04** | **Spike 2：音訊分析實測。R-10 解除；發現取樣率設定會吃掉 A-3 近半預算；修正 §5.2.1 對 R-07 的過度宣稱。** 見下方變更明細。 |
...
本版來源為一份外部架構建議書。七項建議經逐項查證後，**採納四項、部分採納一項、否決兩項**。查證過程同時推翻了建議書的四項引用描述，以及**本規格作者在評估階段自己提出的一項錯誤估算**。

**採納**

1. **【架構變更】LLM 視覺分類提升為主要路徑**（§5.6.3.1、§5.6.3.8）。像素級分類器經 v1.2–v1.4 三輪實測仍無法定出可用門檻，`screen` 類零樣本使自動分類無法啟動。改由管線中既有的 LLM 直接判讀，像素分類器降為離線後備。此路徑不需校準語料，直接使 R-13 失效。
2. **【新增】§4.9 視覺輸入預算**。以官方視覺 token 公式 `tokens = ceil(w/28) x ceil(h/28)` 重新核算抽樣設計（已用官方對照表 4/4 驗證），發現兩項真實限制：單次請求的圖片數上限，以及 **360p 縮圖不足以讓模型判讀 UI 文字**。§4.3 因此改為雙解析度路徑。
3. **【改進】§5.2 節拍吸附加入起音能量權重**。並依 librosa 的實際回傳特性（未正規化、無上界）改用穩健正規化，且修正誤差上界。
4. **【新增】§4.3 場景切換自適應抽樣**（PySceneDetect）。取代固定等距抽樣，避免抽到轉場模糊畫格。
5. **【新增】§4.4 CapCut 草稿適配器層**，隔離 R-05 的格式脆弱性。

**部分採納**

6. **§3.2.3 補上硬體解碼 API 平台對照表**，但**否決建議的「VRAM ≥ 8 GB 即啟用硬體解碼」門檻**——該數值無依據，且 FFmpeg 官方手冊的說明直接反對此方向（見 §3.2.3 引文）。

**否決**

7. **記憶體上限動態放大**（建議：`min(0.5 x RAM_total, 6144 MB)`）。與驗收標準 A-2 直接衝突，且會削弱熔斷器的偵測能力——熔斷器的目的是抓 bug，不是榨乾資源；Chunked 架構為序列處理且有界，多出來的記憶體沒有東西可以擴張進去。改採**保護型**的取小值 `min(3072 MB, 0.25 x RAM_total)`，在受限機器上調低而非在大機器上調高（§6.2）。

**查證發現的引用錯誤**（建議書共 7 項引用，4 項描述與事實不符）

| 引用 | 建議書描述 | 查證結果 |
| :---- | :---- | :---- |
| `Ailln/pycapcut` | CapCut 草稿工具庫 | URL 回傳 404，不存在。實際專案為 `GuanYixuan/pyCapCut` |
| PaddleOCR 檢測模型 | 權重 2–4 MB、延遲 **10–20 ms** | mobile 模型 4–8 MB、**CPU ONNX 延遲 688 ms @ 2048**，差 34–69 倍 |
| `browser-use/video-use` | 參考其「多模態長影片切片理解」 | 該專案**明確拒絕**系統性餵入畫格，主張 text-primary |
| 業界工作流文章 | 「長轉短流程的文本優先與結構化資料流設計」 | 實為口播影片的線性工具鏈，無長轉短亦無結構化資料模型 |

**本規格作者在評估階段的錯誤估算（一併記錄）**

評估建議 2.1 時，作者引用 video-use 的「1500 tokens/圖」推論出「現行抽樣設計會撐爆上下文預算」。**此推論錯誤**：該數字適用於全尺寸畫格，而 §4.3 的縮圖為 360p，實際僅 **104 tokens/張**；10 支素材 80 張合計 8,320 tokens，成本約 0.042 美元，不構成任何限制。此錯誤與 §5.6.3.6 所禁止的行為同型——引用未經查證的數字並外推——故一併記入本節，並依 §5.6.3.6 補上規範：**視覺 token 預算必須以供應商的 token 計數端點實測，不得估算**。

**v1.6 相對 v1.5 的變更明細**：

取得 `screen` 類語料（PowerShell、IDE、手機社群瀏覽、遊戲實機，共 4 支）後，得以首次量測召回面。結果同時**修正了一個持續四個版本的錯誤**，並**推翻可讀性模型的核心假設**。

**1. 根因：一個相依套件的預設參數（§5.6.3.9）**

`cv2.MSER_create()` 的 `min_area` 預設為 **60**。在 1280 代理解析度下，IDE 程式碼文字約 9 px 高、5 px 寬，面積約 45 px——**低於門檻，MSER 根本不回傳這些區域**。

此單一參數同時汙染了 v1.2–v1.5 的多項結論：

| 受影響結論 | 錯誤值（min_area=60） | 修正值（min_area=15） |
| :---- | :---- | :---- |
| 分類器判別力 | AUC 0.929（單來源自助估計） | **AUC 0.983** |
| 最佳門檻與表現 | 4.9，特異度 0.80 | **1.26，召回 1.00 / 特異度 0.95** |
| 共線群組召回 | IDE 保留率 0%，判定否決 | **21–23%，判定採用** |
| IDE 元件偵測數 | 63 | **210**（3.3 倍） |
| 合成文字測試 | 各字級皆回傳 0，判定為「合成影像假象」 | 同一參數問題，該判定亦錯誤 |

**2. 共線群組約束：由「否決」改為「採用」（§5.6.3.5）**

v1.5 記載其「召回面未測量、可能過於激進」。實測後**確認有效**：screen 三支保留率一致（21–23%），natural 中位僅 11%，最大值由 23.54 壓至 3.85。判別力 AUC 由 0.950 提升至 **0.983**，特異度 0.90 → 0.95。先前的否證測試用的是被癱瘓的參數，結論不成立。

**3. 可讀性模型的 φ 假設被推翻（§5.6.2）**

v1.5 假設 IDE 的 $\varphi = 0.0176$。實測結果：

| 素材 | 實測 $\varphi$ | 滿高 9:16 裁切後字高 | 判定 |
| :---- | :---- | :---- | :---- |
| PowerShell | 0.0172 | 33.1 px | 勉強可讀 |
| **IDE** | **0.0108** | **20.7 px** | **低於門檻** |
| 手機社群 | 0.0054 | 10.4 px | 低於門檻 |

假設值樂觀了 63%。**這改變了 §5.6.2 的結論**：原本寫「必須放棄約 68% 的畫面內容」，實測顯示真實 IDE 錄影要達到最低可讀門檻只能顯示 **16.4%** 的畫面，達到舒適門檻則僅 **8.0%**。詳見 §5.6.2.1。

**4. 兩類 v1.5 未處理的素材型態（§4.3.1、§5.6.9）**

* **可變幀率（VFR）**：實測四支螢幕錄影的回報幀率為 7.4 / 22.7 / 48.2 / 115.3 fps——螢幕錄影器對靜態畫面降幀。`CAP_PROP_POS_FRAMES` 在 VFR 上定位不可靠，須改用 `POS_MSEC`。
* **直式來源**：手機錄影為 1080×2400（長寬比 0.45），**比目標 9:16 更長**。現行重構模式全部假設來源較寬，此情況需要反向處理。

**5. 架構定位調整（§5.6.3.8）**

v1.5 將 LLM 分類提升為主要路徑，理由是「像素路徑經三輪實測仍不可用」。**該前提現已不成立**——像素路徑不可用是參數錯誤造成的。兩條路徑現皆可行，故改為**並列評測**，於 M2 以同一語料決定主從，不預設立場。

> **樣本數警告**：本次 `screen` 語料僅 **3 支**（遊戲屬第三類）。AUC 0.983 建立在 3 vs 20 之上，依 §5.6.3.6 為 **B 級證據**。門檻 1.26 為暫定值。

> **方法論教訓（第二次）**：v1.4 記錄過「合成樣本不可作為門檻依據」。本次的教訓不同且更基本——**一個相依套件的預設參數，可以讓四輪實測全部得出錯誤結論，而每一輪的內部邏輯都是自洽的**。故 §5.6.3.6 新增規範：任何以第三方函式庫為基礎的量測，必須先驗證該函式庫的預設參數對本專案的輸入型態是否適用。

**v1.7 相對 v1.6 的變更明細**：

FFmpeg 安裝完成後執行 Spike 1，**首次實際執行 §5.3／§5.5／§5.6.8 的濾鏡鏈**——這些條文自 v1.0 起從未跑過。7 項測試全數通過，但暴露一項模型錯誤與數項規格細節問題。

**驗證通過（規格假設成立）**

| 測試 | 結果 |
| :---- | :---- |
| §5.3.1 `blur_padding` 濾鏡鏈 | 輸出 1080×1920 / yuv420p / 30fps，完全符合 |
| §5.5 影格精確切片（T-04） | 切 3.0 秒 @30fps = **實際 90 幀 / 預期 90 幀，誤差 0** |
| T-06 混合幀率拼接 | 24/25/60fps 三段正規化後拼接，**時長漂移 0.0 ms**（門檻 100 ms） |
| §3.2.2 編碼器冒煙測試 | 正確選出 `h264_nvenc`；`h264_amf` 雖編入建置但實測失敗並被拒絕 |
| §3.2.4 硬體編碼品質參數 | `h264_nvenc`、`h264_qsv` 均可用 |

**§5.5 的驗證特別值得記錄**：v1.0 曾宣稱「`-ss` 前置會導致切點飄移」，v1.1 依 FFmpeg 文件將其修正為「僅在搭配 `-c copy` 時成立」。本次實測**誤差為 0 幀**，v1.1 的修正得到證實。

**發現一：記憶體模型低估 1.85 倍（§5.4）**

| 項目 | v1.6 模型 | Spike 1 實測 |
| :---- | :---- | :---- |
| 4K chunk 峰值 RSS | 655 MB | **1211 MB** |
| 相對熔斷上限（3072 MB）的餘裕 | 4.5 倍 | **2.5 倍** |

模型未計入編碼器的前瞻緩衝（lookahead）。`libx264 -preset medium` 的 `rc-lookahead` 預設約 40 幀，在 1080×1920 下即約 124 MB，且 `split` 會使 4K 解碼串流複製為兩路。詳見 §5.4.4。

熔斷上限 3072 MB 仍然足夠，但餘裕比宣稱的小得多。

**發現二：模糊最佳化的增益遠小於宣稱（§5.3.1）**

v1.6 §5.3.1 稱「先縮小 → 模糊 → 放大」可「**大幅降低運算量**」。實測：

| 指標 | BLUR_FULL | BLUR_FAST | 差異 |
| :---- | :---- | :---- | :---- |
| 耗時（5 秒 4K 片段） | 5.19 s | 3.95 s | **1.31 倍** |
| 峰值 RSS | 1213 MB | 1205 MB | **無差異** |
| SSIM（相對 BLUR_FULL） | — | 0.974 | — |

1.31 倍是真實增益但非「大幅」，且**完全不改善記憶體**。判定：採用，但修正宣稱用語。畫質經 SSIM 0.974 與目視比對無明顯差異，惟測試素材為 `testsrc2` 合成圖樣（平坦色塊），依 §5.6.3.6 規範**不得以合成樣本拍板畫質**，須以真實素材複驗。

**發現三：硬體編碼同時降低記憶體（新增觀察）**

| 編碼器 | 耗時 | 峰值 RSS |
| :---- | :---- | :---- |
| `libx264 -preset medium` | 5.19 s | 1213 MB |
| `h264_nvenc` | **3.41 s** | **762 MB** |
| `h264_qsv` | 4.53 s | **628 MB** |

硬體編碼不僅快 1.52 倍，峰值記憶體亦低 **37%**——編碼緩衝移至 GPU 側。此為 §5.4 未預期的效益，使 `hw_accel=auto` 的預設更具正當性。

**發現四：驗收 A-6 的首個真實數據點**

5 秒 4K→1080×1920 以 `h264_nvenc` 渲染耗時 3.41 秒（含進程啟動）。外推至 60 秒成品約需 **41 秒**，對照 A-6 門檻（≤ 90 秒）有充分餘裕。此為單段外推，實際分段數更多時的進程啟動成本須於 M3 複測。

**發現五：安裝指引錯誤（§3.3、README）**

* winget 套件 ID **`Gyan.FFmpeg.Full` 不存在**，正確為 **`Gyan.FFmpeg`**。
* 實測安裝版本為 **9.0.1**，超出 §3.3 所寫的「6.1+ 或 7.x」兩個大版本。所有濾鏡鏈在 9.0.1 上運作正常。
* winget 安裝後**既有 shell 的 `PATH` 不會即時生效**，需重開 shell。`check_env.py` 已加入此提示。

**v1.8 相對 v1.7 的變更明細**：

安裝 librosa 後執行 Spike 2，首次實際執行 §4.2 節拍偵測與 §5.2／§5.2.1 節拍吸附。最終 7/7 通過，但過程中修正兩項規格缺陷與**一項作者的錯誤解讀**。

**R-10 解除**

`librosa 1.0.0` + `numba 0.67.0` 在 **Python 3.12** 上正常運作。R-10（相依套件版本衝突）的具體疑慮已排除。注意 librosa 已進入 **1.0 大版本**，§3.3 的下限 `>=0.10.2` 需更新。

**缺陷一：分析取樣率吃掉 A-3 近半預算（§4.2.1）**

節拍時間戳的時間解析度為 `hop_length / sr`。librosa 的 `load()` 預設重取樣至 **22050 Hz**，在 `hop_length=512` 下解析度為 **23.2 ms**——佔驗收標準 A-3（切點偏差 ≤ 50 ms）的 **46%**。

改以 §4.1 既有的 44.1 kHz WAV 直接載入後：

| 組態 | 解析度 | 平均間隔法 BPM | 節拍間隔標準差 | 佔 A-3 預算 |
| :---- | :---- | :---- | :---- | :---- |
| sr=22050, hop=512（librosa 預設） | 23.2 ms | 119.99 | 0.0116 s | **46%** |
| **sr=44100, hop=512（採用）** | **11.6 ms** | **120.00** | **0.0029 s** | **23%** |
| sr=44100, hop=256 | 5.8 ms | 120.00 | — | 12% |

節拍間隔標準差改善 **4 倍**。此為零成本修正——§4.1 產出的本來就是 44.1 kHz WAV，先前是白白重取樣降級。

**缺陷二：BPM 不得取自 librosa 的 `tempo` 標量（§4.2）**

實測發現 `beat_track` 回傳的 `tempo` 標量在 sr=22050 下為 **117.45**（真值 120，偏差 2%），而由**平均**節拍間隔計算為 119.99。

根因：`hop_length` 量化使節拍間隔在 21 幀（0.4876 s）與 22 幀（0.5108 s）之間跳動；`tempo` 標量對應的是**中位**間隔（0.5108 s → 117.45），而平均間隔才是無偏估計。

**規範**：`analyze_audio_beats` 回傳的 `bpm` **必須**由 `60 / mean(diff(beat_timestamps))` 計算。實測顯示此法在所有取樣率組態下皆準確（119.99–120.02）。

**缺陷三：§5.2.1 對 R-07 的宣稱過度（§5.2.1）**

v1.5 引入起音能量加權時宣稱「重拍通常正是 onset 能量高的拍點，故能量加權可**免費取得部分 downbeat 效果**，不必引入 madmom」。

以帶重音的 4/4 合成素材（重拍能量 21.4 vs 弱拍 13.3，週期性比 1.95）實測 300 次重抽：

| $\gamma$ | 切點命中重拍比例 | 誤差上界 |
| :---- | :---- | :---- |
| 0.0（停用） | 27.0% ± 9.1% | 0.250 s |
| **0.3（預設）** | **30.1% ± 10.0%** | 0.399 s |
| 0.6 | 33.3% ± 10.6% | 0.549 s |
| 1.2 | 44.1% ± 10.2% | 0.849 s |

（隨機基準 25%）

效果**方向正確且隨 $\gamma$ 單調遞增**，但在可用範圍內幅度僅約 +3～5 個百分點。

**結構性原因**：$\beta$ 的作用半徑就是 $\beta$ 秒。在 120 BPM 下，$\gamma=0.3$ 給出 $\beta = 0.15$ s，僅能跨越 **0.30 個節拍間隔**；而 4/4 拍的重拍間距是 **4 個**節拍間隔（2.0 s）。要讓 $\beta$ 觸及重拍需 $\gamma \ge 4$，此時誤差上界達 2.25 s，時間精度完全崩潰。

**修正**：保留能量加權，但**移除其作為 R-07 替代方案的宣稱**。它的正當用途是「在鄰近候選中偏好能量較高者」以增強卡點衝擊感，**不是**重拍對齊。R-07 仍未解決。

**作者的錯誤解讀（一併記錄）**

首次 Spike 2 執行以 $n=25$ 單次抽樣得到「$\gamma=0$ 為 36%、$\gamma=0.3$ 為 28%」，作者據此在對話中判定「能量加權使結果變差」。**此判定錯誤**——該組態的標準差約 10%，$n=25$ 不足以分辨 3 個百分點的差異。改為 300 次重抽後方向為正。

此與 §5.6.3.6 的教訓同型：**樣本數不足時，單次量測的方向本身就是雜訊**。已將重抽次數要求納入 §8.3 測試規範。

**驗證通過的項目**

| 測試 | 結果 |
| :---- | :---- |
| T-02 吸附不漂移 | 20 個鏡頭，終點誤差 **0.00 s**，全部切點嚴格落在節拍上 |
| §5.2.1 誤差上界 | 最大誤差 0.235 s，上界 0.400 s（$\gamma=0.3$），公式成立 |
| §5.2.1 穩健正規化 | 注入 12 倍離群值：`E/E_max` 均值塌陷 0.746→0.070；百分位法 0.516→0.516（**完全不受影響**） |
| §5.2.1 `hop_length` 一致性 | 刻意錯配時最大錯位 2563 幀（約 29.8 s），足以取到完全無關的能量值——證實該規範必要 |

穩健正規化的實測特別值得記錄：外部建議書原式 `E/E_max` 在單一離群值下均值塌陷 **90%**，證實 v1.5 依 librosa 文件「回傳值未正規化、無上界」所做的修正是必要的。
...
| 能力 | 需求說明 | 用途 |
| :---- | :---- | :---- |
| **Vision（多圖輸入）** | 單次請求可接收 ≥ 20 張拼貼縮圖（每張約 2574×1446），且單張視覺 token 上限 ≥ 4800 | 讀取 §4.3 產出的素材縮圖，判斷內容類別與可用性。尺寸需求的推導見 §4.9 |
...
**v1.5 補充：FFmpeg 官方手冊直接支持此決策**

一份外部建議提議「可調用 VRAM ≥ 8 GB 即啟用硬體解碼」。該門檻無依據來源，且 FFmpeg 官方手冊對 `-hwaccel` 的說明直接反對此方向：

> most acceleration methods are intended for playback and will not be faster than software decoding on modern CPUs. Additionally, ffmpeg will usually need to copy the decoded frames from the GPU memory into the system memory, resulting in further performance loss.
>
> —— FFmpeg 官方手冊，`-hwaccel` 章節

兩點與本專案直接相關：① 硬體解碼在現代 CPU 上**未必更快**；② GPU→系統記憶體的畫格回拷會造成額外損耗，而本專案的 chunk 預裁切必然需要把畫格送回 CPU 端做濾鏡處理。

因此**否決該建議**。正確的把關方式不是固定 VRAM 數值，而是延用 §3.2.2 既有的冒煙測試機制——實測能跑且更快才啟用。若日後開放，須先以 §8.4 的 A-6 效能驗收證明其確有增益。

**硬體解碼 API 對照表**（`-hwaccel` 可接受值，取自官方手冊）

| 值 | 說明 | 適用平台 |
| :---- | :---- | :---- |
| `none` | 不使用硬體加速（**預設值**） | 全平台 |
| `auto` | 自動選擇 | 全平台 |
| `d3d11va` | D3D11VA (DirectX Video Acceleration) | Windows（較新，優於 dxva2） |
| `dxva2` | DXVA2 (DirectX Video Acceleration) | Windows（舊式） |
| `vaapi` | VAAPI (Video Acceleration API) | Linux |
| `vdpau` | VDPAU | Unix（多為 NVIDIA 舊式路徑） |
| `qsv` | Intel QuickSync Video | Windows / Linux（Intel） |
| `videotoolbox` | Video Toolbox | macOS |

> 注意：`-hwaccel` 控制**解碼**，與 §3.2.2 的 `-c:v` **編碼器**選擇是兩組獨立設定，名稱相近但不可混用（例如 `qsv` 兩邊都有，但語意不同）。
...
* **FFmpeg / FFprobe**: **6.1 以上**，需為含硬體編碼器的完整建置。
  * Windows 安裝：`winget install --id Gyan.FFmpeg`（**注意：`Gyan.FFmpeg.Full` 這個 ID 不存在**）。
  * **實測版本 9.0.1**（gyan.dev full build），§5.3 / §5.5 / §5.6.8 全部濾鏡鏈在此版本上驗證通過。
  * 安裝後**須重開 shell** 才會套用 `PATH`。
...
| `librosa` | `>=0.10.2` | 音訊訊號處理與節拍萃取。**實測 1.0.0 + numba 0.67.0 於 Python 3.12 正常**（R-10 解除）。載入時**必須顯式指定 `sr=44100`**，勿用預設重取樣（§4.2.1） |
...
| `scenedetect` | `>=0.6.4` | §4.3 場景切換自適應抽樣。BSD-3-Clause，相依 OpenCV（已具備），無額外重量級相依 |
...
#### 4.2.1 時間解析度與 A-3 預算（v1.8 新增）

節拍時間戳的時間解析度由 `hop_length / sr` 決定。此值直接消耗驗收標準 **A-3**（成品切點與最近節拍點偏差 ≤ 50 ms）的預算：

| 組態 | 解析度 | 佔 A-3 預算 | 平均間隔法 BPM | 節拍間隔標準差 |
| :---- | :---- | :---- | :---- | :---- |
| sr=22050, hop=512（**librosa `load()` 預設**） | 23.2 ms | **46%** | 119.99 | 0.0116 s |
| **sr=44100, hop=512（規範值）** | **11.6 ms** | 23% | **120.00** | **0.0029 s** |
| sr=44100, hop=256 | 5.8 ms | 12% | 120.00 | — |

**規範**

1. **載入時必須顯式指定 `sr=44100`**。`librosa.load()` 預設會重取樣至 22050 Hz——而 §4.1 產出的本來就是 44.1 kHz WAV，使用預設等於白白降級。此修正零成本，節拍間隔標準差改善 4 倍。
2. `hop_length` 由 `[beat] hop_length` 設定，預設 512。若 A-3 實測餘裕不足，降至 256 可再減半解析度誤差，代價是 onset 包絡線長度加倍。
3. **`bpm` 必須由 `60 / mean(diff(beat_timestamps))` 計算，不得採用 `beat_track` 回傳的 `tempo` 標量。**

**為何不能用 `tempo` 標量**：實測在 sr=22050 下 `tempo` 回報 **117.45**（真值 120，偏差 2%）。根因是 `hop_length` 量化使節拍間隔在 21 幀（0.4876 s）與 22 幀（0.5108 s）間跳動；`tempo` 對應**中位**間隔（偏向較常出現的 22 幀），而**平均**間隔才是無偏估計。實測平均間隔法在所有取樣率組態下皆準確（119.99–120.02）。
...
v1.4 以前，360p 縮圖同時服務兩個用途。§4.9 的核算顯示這兩個用途的解析度需求**互相衝突**，故拆為兩條路徑：

| 路徑 | 解析度 | 用途 | 理由 |
| :---- | :---- | :---- | :---- |
| **像素分析代理** | 寬 360 px（`scale=360:-2`） | §5.6.3 訊號計算、§5.6.4 活動熱區 | 維持 §2.2 抽樣降規隔離，防 OOM |
| **LLM 判讀縮圖** | 寬 `[llm] thumb_width`（預設 858 px） | 送交 LLM 的視覺輸入 | 360 px 下 1080p 螢幕錄影的 UI 文字只剩 3.6 px，模型無法判讀（§4.9） |

兩路徑抽取**同一批時間點**的畫格，僅縮放尺寸不同，故不增加解碼次數。

**v1.6 變更：可變幀率（VFR）與直式來源**

實測四支螢幕錄影的回報幀率為 **7.4 / 22.7 / 48.2 / 115.3 fps**——螢幕錄影器普遍採可變幀率，對靜態畫面大幅降幀。這造成兩個問題：

1. **幀定位不可靠**：`CAP_PROP_POS_FRAMES` 在 VFR 上的行為未定義。**規範：一律以 `CAP_PROP_POS_MSEC` 依時間定位**，並以 `duration_ms × (i + 0.5) / k` 計算取樣點。
2. **回報幀率不可信**：`CAP_PROP_FPS` 在 VFR 上回傳的是平均值，不可用於時間換算。時長應以容器中繼資料為準。

渲染階段不受影響——§5.5 的 `-r 30` 已強制輸出為固定幀率（CFR）。

**直式來源（長寬比 < 0.5625）**

手機錄影實測為 1080×2400（長寬比 **0.45**），**比目標 9:16（0.5625）更長**。§5.3 的所有重構模式均假設來源較寬，此情況需反向處理：

| 情況 | 處理 |
| :---- | :---- |
| 長寬比 ≈ 0.5625（±0.02） | 直接縮放，無需重構 |
| 長寬比 < 0.5625（更長） | 垂直裁切至 9:16。**注意：此時縮放比為 1.0，文字不會放大**（實測手機素材 13 px 進、13 px 出），可讀性完全取決於來源 |
| 長寬比 > 0.5625（較寬） | §5.3 現行各模式 |

**規範**：`probe_and_sample_media` 須回報 `orientation` 欄位（`portrait_taller` / `portrait_match` / `landscape`），供 `plan_reframe` 選用正確路徑。

**v1.5 變更二：場景切換自適應抽樣**

固定等距抽樣在含多次鏡頭切換的長素材上，可能抽到轉場的模糊畫格。此類畫格同時汙染兩件事：LLM 看到的視覺輸入，以及 §5.6.3.2 的素材層級中位數聚合。

改採 [PySceneDetect](https://github.com/Breakthrough/PySceneDetect)（BSD-3-Clause）的 `ContentDetector` 先行分鏡，再自各鏡頭時間中點抽取代表幀：

1. 以 `ContentDetector` 掃描素材，取得鏡頭清單。
2. 鏡頭數 $\ge k$ 時，取時長最長的 $k$ 個鏡頭，各抽其時間中點 1 幀。
3. 鏡頭數 $< k$ 時，於各鏡頭內等距補足至 $k$ 幀。
4. 偵測失敗或未偵測到切換時，退回固定等距抽樣。

**效能規範**：`SceneManager` **必須**設定 `auto_downscale = True`（或顯式 `downscale`）。掃描全片是此步驟的主要成本，4K 原生解析度下不可接受，且違反 §2.2 抽樣降規隔離。

> **不得使用 `frame_skip`**：官方文件明載該參數「Not recommended except for extremely high framerate videos」，且與 `StatsManager` 併用會拋出 `ValueError`。降解析度才是正確的效能旋鈕。

**螢幕錄影的例外**：螢幕內容的「場景」語意不同——捲動不是切換，切換分頁才是。`ContentDetector` 對此類素材可能過度分段。當 §5.6.3 判定為 `screen_recording` 時，改用較高的 `threshold` 或直接退回等距抽樣（由 `[sampling] scene_detect_on_screen` 控制，預設 `false`）。
...
**4.4.1 適配器層架構（v1.5 新增）**

為隔離上述格式脆弱性，軌道 A 採適配器模式（Adapter Pattern）：

```
[AutoShorts EDL 模型] --> [CapCutAdapter (抽象介面)] --> [具體後端] --> [草稿目錄]
                                                          |
                                        +-----------------+-----------------+
                                        |                                   |
                              [內建產生器（自維護）]              [社群函式庫後端]
...
**規範**：EDL → 草稿的映射邏輯只寫一次，位於適配器介面之上；後端可替換。這使得社群函式庫失效時，切換成本侷限於單一模組。

> **依賴社群函式庫是「轉移」風險，不是「消除」風險。** 若該函式庫停止維護，本專案同時繼承格式問題**與**一個死掉的依賴。R-05 真正的安全網仍是軌道 B 這條完全獨立的路徑。

**4.4.2 候選後端評估（v1.5 查證結果）**

| 專案 | 目標軟體 | 授權 | 狀態 | 可否採用 |
| :---- | :---- | :---- | :---- | :---- |
| [`GuanYixuan/pyJianYingDraft`](https://github.com/GuanYixuan/pyJianYingDraft) | 剪映（中國版） | **Apache-2.0** | 較成熟 | ✅ 授權可用 |
| [`GuanYixuan/pyCapCut`](https://github.com/GuanYixuan/pyCapCut) | CapCut（國際版） | **無授權檔** | 18 commits，README 自述「正在從 pyJianYingDraft 遷移」 | ❌ **阻擋** |

**關鍵發現**

1. **`pyCapCut` 未載明授權。** GitHub 的 license API 對該倉庫回傳 404，倉庫頁面亦無授權資訊（兩次獨立確認）。**無授權等同保留所有權利**，在作者補上授權前不得採用。
2. **CapCut 國際版與剪映中國版是兩個分離的專案。** 同一作者維護兩套程式碼庫，直接證實兩者草稿格式不同——此即附錄 A-2 所詢問的問題，現已有答案：**必須分別支援，不能視為同一格式**。
3. `pyCapCut` 產生的草稿**仍需在 Windows 版 CapCut 中匯出**，非純離線流程。
4. 功能面對軌道 A 是足夠的：音訊軌、關鍵影格、轉場均支援；已知不支援曲線變速（本專案 v1.0 不需要）。

**M3-A 的後端選用決策**依下列順序：① 若 `pyCapCut` 補上相容授權則評估採用；② 否則以 `pyJianYingDraft`（Apache-2.0）支援剪映路徑，CapCut 路徑改用內建產生器；③ 兩者皆不可行時，僅交付內建產生器。

**外部建議書的引用錯誤**：該建議書引用的 `Ailln/pycapcut` URL 回傳 404，並不存在。正確專案如上表。
...
| `SOURCE_` | 素材本身的限制 | `SOURCE_TEXT_TOO_SMALL`（§5.6.2.1，幾何上無法補救，須來源端調整）, `SOURCE_ORIENTATION_UNSUPPORTED` |
...
### 4.9 視覺輸入預算 (Visual Input Budget)（v1.5 新增）

送交 LLM 的縮圖數量與尺寸，受三項獨立限制約束。本節建立核算方法，並記錄一項因未核算而產生的錯誤判斷。

**4.9.1 視覺 token 計價模型**

模型以 28×28 像素區塊（patch）為單位處理影像，故：

$$\text{tokens} = \left\lceil \frac{w}{28} \right\rceil \times \left\lceil \frac{h}{28} \right\rceil$$

超過解析度層上限的影像會先等比縮小再計算。已用官方對照表驗證，4/4 完全吻合：
...
以 §4.3 的 360p 縮圖（360×202）計：

| 素材數 | 縮圖數 | 視覺 tokens | 概略成本（5 美元/MTok） |
| :---- | :---- | :---- | :---- |
| 5 | 40 | 4,160 | 0.021 美元 |
| 10 | 80 | 8,320 | 0.042 美元 |
| 20 | 160 | 16,640 | 0.083 美元 |

**結論：視覺 token 的成本與上下文佔用都不構成限制。** 單張 360p 縮圖僅 104 tokens。

**4.9.3 拼貼縮圖不省 token**

外部建議提出以九宮格拼貼降低 token 成本。核算顯示**此效益不存在**：
...
因 token 成本與**像素面積**成正比，拼貼後總面積不變，只省下極小的每張固定開銷。

**4.9.4 真正的限制一：單次請求圖片數上限**

| 限制 | 數值 |
| :---- | :---- |
| 單次請求圖片數上限（200K 上下文模型） | 100 張 |
| 單次請求圖片數上限（其他模型） | 600 張 |
| 超過 20 張時 | 每張另受更嚴格的尺寸限制 |

| 素材數 × 8 張 | 總張數 | 狀態 |
| :---- | :---- | :---- |
| 5 | 40 | 觸發 >20 張的嚴格尺寸限制 |
| 10 | 80 | 觸發 >20 張的嚴格尺寸限制 |
| 20 | 160 | **超過 100 張上限** |

**拼貼縮圖的正確理由是這一項，不是 token 成本。** 每支素材拼成 1 張後，20 支素材僅 20 張，全數落在安全區。

**4.9.5 真正的限制二：360p 對模型不足以判讀 UI 文字**

以 §5.6.2 的 $\varphi$ 模型計算——1080p 螢幕錄影的 IDE 文字（$\varphi = 0.0176$，原始 19 px）在不同縮圖寬度下的殘存字高：

| 縮圖寬 | 尺寸 | 殘存字高 | tokens/張 | 模型可判讀性 |
| :---- | :---- | :---- | :---- | :---- |
| 360 | 360×202 | **3.6 px** | 104 | 無法辨識 |
| 640 | 640×360 | 6.3 px | 299 | 無法辨識 |
| 768 | 768×432 | 7.6 px | 448 | 無法辨識 |
| **858** | 858×482 | **8.5 px** | 558 | 勉強可辨 |

官方文件亦警告模型「可能在很小的影像（低於 200 像素）上出錯」。

> **這直接影響 §5.6.3.8 的 LLM 分類可行性**：要模型判斷「這是不是螢幕錄影」，它必須看得見畫面上有 UI 文字。3.6 px 下連「這是文字」都無從判斷。§4.3 的雙解析度路徑即為此而設。

**4.9.6 規範**

1. LLM 判讀縮圖寬度預設 **858 px**，拼貼為每支素材 1 張（3×3，最多 9 幀）。拼貼後尺寸 2574×1446，恰達高解析度層上限 4784 tokens/張；10 支素材約 47,840 tokens，成本約 0.24 美元。
2. 像素分析代理維持 360 px 不變。
3. **視覺 token 用量必須以供應商的 token 計數端點實測**（Claude 為 `POST /v1/messages/count_tokens`），不得以每張固定 token 數估算。不同供應商的計價模型差異極大。

> **本節的由來（記錄一次錯誤）**：本規格作者在評估外部建議時，援引某開源專案宣稱的「1500 tokens/圖」，推論出「現行抽樣設計會撐爆上下文預算」。實際核算後該推論**完全錯誤**——該數字適用於全尺寸畫格，360p 縮圖僅 104 tokens，差 14 倍。此為 §5.6.3.6 所禁止之行為（引用未經查證的數字並外推）的又一實例，故將核算方法固化為本節規範。
...
#### 5.2.1 起音能量加權吸附（v1.5 新增）

**動機**：上述演算法只看時間距離，會把切點吸附到「最近」的節拍，而不是「聽起來最有衝擊力」的節拍。在容許誤差內偏向能量峰值，可明顯增強卡點的節奏感。

**加權吸附式**

$$T_k = \arg\min_{b \in C_k} \Bigl( \bigl| b - \hat{T}_k \bigr| - \beta \cdot \tilde{E}(b) \Bigr)$$

其中 $\tilde{E}(b) \in [0, 1]$ 為節拍點 $b$ 的正規化起音強度，$\beta$ 為偏置強度（單位：秒）。

**$\beta$ 必須相對於節拍間隔**

$$\beta = \gamma \cdot \frac{60}{\text{BPM}}, \qquad \gamma \in [0.2,\ 0.4]\ (\text{預設 } 0.3)$$

固定秒數的 $\beta$ 會使慢歌偏置過度、快歌偏置不足。在 128 BPM 下，$\gamma \in [0.2, 0.4]$ 對應 $\beta \approx 0.094 \sim 0.188$ 秒。

**正規化必須是穩健的（關鍵實作要求）**

librosa 官方文件記載 `onset_strength` 的回傳值為：

> **onset_envelope**: np.ndarray，spectral flux 值，**非負、無上界、未正規化**

因此**不得**採用 $\tilde{E}(b) = E(b) / E_{\max}$。單一離群峰值（如一次鈸擊）就會把其餘所有節拍壓到接近 0，使能量項失效——這是外部建議書原式的缺陷。

規範採**節拍點上的百分位正規化**：

$$\tilde{E}(b) = \operatorname{clip}\left( \frac{E(b) - P_{10}}{P_{90} - P_{10}},\ 0,\ 1 \right)$$

其中 $P_{10}$、$P_{90}$ 為**僅在節拍點集合 $B$ 上**取得的起音強度百分位（非整條包絡線）。若 $P_{90} - P_{10} < \epsilon$（能量分布過於平坦），則令 $\tilde{E} \equiv 0$，退化回純時間吸附。

**索引對齊（實作陷阱）**

`onset_strength` 回傳的是逐畫格包絡線，`beat_track` 回傳的是節拍畫格索引。兩者**必須以同一個 `hop_length` 呼叫**，否則索引無法對齊、取到錯誤的能量值。

**規範**：`hop_length` 由設定檔顯式指定並傳給兩個函式，不得依賴各自的預設值。時間換算使用 `librosa.frames_to_time(frames, sr=sr, hop_length=hop_length)`。

**誤差上界改寫**

能量偏置會使被選中的節拍可能比最近者更遠，故 §5.2 原本的上界不再成立，改為：

$$\bigl| c_k - \hat{c}_k \bigr| \le \frac{1}{2} \cdot \frac{60}{\text{BPM}} + \beta = \left( \frac{1}{2} + \gamma \right) \cdot \frac{60}{\text{BPM}}$$

在 128 BPM、$\gamma = 0.3$ 下，上界由 0.234 秒放寬至 **0.375 秒**。此值仍遠低於驗收標準 A-3 所要求的「切點與最近節拍點偏差 ≤ 50 ms」——因為 A-3 量的是切點**是否落在節拍上**，而非是否落在「最近的」節拍上。加權吸附後切點仍嚴格落在某個節拍點上，A-3 不受影響。

**與 R-07 的關聯：v1.8 撤回原宣稱**

v1.5 曾宣稱「重拍通常正是起音能量較高的拍點，故能量加權可在不引入 `madmom` 的前提下取得部分重拍對齊效果」。**此宣稱經實測後撤回。**

以帶重音的 4/4 合成素材（重拍能量 21.4 vs 弱拍 13.3，4 拍週期性比 1.95）實測 300 次重抽：
...
隨機基準 25%。效果方向正確且隨 $\gamma$ 單調遞增，但在可用範圍內僅 +3～5 個百分點。

**結構性限制**：$\beta$ 的作用半徑就是 $\beta$ 秒。120 BPM 下 $\gamma = 0.3$ 給出 $\beta = 0.15$ s，僅能跨越 **0.30 個節拍間隔**；而 4/4 的重拍間距是 **4 個**節拍間隔（2.0 s）。要讓 $\beta$ 觸及重拍需 $\gamma \ge 4$，屆時誤差上界達 2.25 s，時間精度完全崩潰。

$$\text{作用半徑} = \frac{\beta}{\text{節拍間隔}} = \gamma \quad \text{（以節拍數計）}$$

即 $\gamma$ 直接就是「能跨越幾個節拍」。要跨越 $m$ 拍的重拍間距，需 $\gamma \ge m$——而誤差上界為 $(1/2 + \gamma) \cdot 60/\text{BPM}$，兩者無法同時滿足。

**結論**：能量加權的正當用途是「在鄰近候選中偏好能量較高者」以增強卡點衝擊感，**不是重拍對齊**。**R-07 仍未解決**，若需可靠的重拍偵測，仍須面對 §11.2 的 `madmom` 授權問題或其他方案。

**證據等級**（v1.8 更新）

| 項目 | 等級 | 依據 |
| :---- | :---- | :---- |
| 穩健正規化的必要性 | **A** | 實測 `E/E_max` 在單一 12 倍離群值下均值塌陷 90%（0.746→0.070），百分位法完全不受影響（0.516→0.516） |
| `hop_length` 一致性要求 | **A** | 實測刻意錯配時最大錯位 2563 幀（約 29.8 s） |
| 誤差上界公式 | **A** | 實測最大誤差 0.235 s < 上界 0.400 s |
| 加權對重拍對齊的效果 | **B** | 合成素材 300 次重抽；效果存在但幅度小，且已證實結構性受限 |
...
> **效能最佳化（v1.1 提出，v1.7 實測後採用）**：在 1080x1920 全解析度上執行 `boxblur=luma_radius=25` 成本較高，而模糊結果本來就不需要高頻細節。改採「先縮小 → 模糊 → 放大」：
>
> ```
...
> ```
>
> **v1.7 實測結果**（5 秒 4K 片段，`libx264 -preset medium`）：
>
> | 指標 | BLUR_FULL | BLUR_FAST |
> | :---- | :---- | :---- |
> | 耗時 | 5.19 s | **3.95 s（1.31 倍）** |
> | 峰值 RSS | 1213 MB | 1205 MB（**無改善**） |
> | SSIM | — | 0.974 |
>
> **判定：採用。** 但 v1.1 原文所稱「大幅降低運算量」不成立——實際為 1.31 倍，且對記憶體毫無幫助。
>
> ⚠️ **畫質尚未定案**：SSIM 0.974 與目視比對均無明顯差異，但測試素材為 `testsrc2` 合成圖樣（平坦色塊），正是粗糙模糊最不易露餡的內容。依 §5.6.3.6 規範，**須以真實攝影素材複驗後才可鎖定**。
...
Spike 1 實測 5 秒 4K 片段經 `BLUR_FULL` 濾鏡鏈以 `libx264 -preset medium` 編碼：

| 項目 | §5.4.2 模型 | **實測** | 比值 |
| :---- | :---- | :---- | :---- |
| 單一 chunk 進程峰值 RSS | 255 MB | **1211 MB** | **4.75 倍** |
| 全系統峰值（模型含 orchestrator 400 MB） | 655 MB | 1211 MB | 1.85 倍 |
| 相對熔斷上限 3072 MB 的餘裕 | 4.5 倍 | **2.5 倍** | — |

**模型遺漏的項目**

1. **編碼器前瞻緩衝（lookahead）**——最大的單一遺漏。`libx264 -preset medium` 的 `rc-lookahead` 預設約 40 幀；在 1080×1920 yuv420p 下每幀 3.1 MB，即約 **124 MB**，且實際實作會另持有分析用的降採樣副本。
2. **`split` 使解碼串流複製為兩路**——§5.4.2 只計了一份 4K 解碼緩衝，實際上前景與背景兩條分支各自持有畫格佇列。
3. **FFmpeg 濾鏡間佇列深度大於假設的 8 幀**——實際深度依濾鏡而異，`boxblur` 這類需要鄰域的濾鏡持有更多。
...
硬體編碼將編碼緩衝移至 GPU 側，峰值 RSS 降低約 **37%**。這是 §5.4.2 未預期的效益，也強化了 `hw_accel = "auto"` 作為預設的正當性。

**修正後的規範**

* 熔斷上限維持 `min(3072 MB, 0.25 × RAM_total)`——實測峰值 1211 MB 仍有 2.5 倍餘裕。
* **但 §5.4.2 的解析推導不可再作為容量規劃依據**，僅保留為說明 Chunked 架構為何有界的定性論證。實際容量以本節實測值為準。
* M3-B 須在啟用硬體編碼、且分段數較多的真實情境下複測，確認多次進程啟動不會累積記憶體。

**5.4.3 驗收與熔斷**

* §8.4 驗收以 `psutil` 量測整個進程樹（含子進程）的 RSS 峰值。
* 渲染期實作**記憶體熔斷器**：若進程樹 RSS 超過 `memory_limit_mb`，立即終止當前 chunk 並回傳 `RENDER_CHUNK_FAILED`，附上實測值——寧可失敗也不可拖垮使用者的機器。

**熔斷閾值的計算（v1.5 修訂）**

$$\text{memory\_limit} = \min\bigl( 3072\ \text{MB},\ 0.25 \times \text{RAM}_{\text{total}} \bigr)$$

一份外部建議提議改為 $\min(0.5 \times \text{RAM}_{\text{total}},\ 6144\ \text{MB})$，即在大記憶體機器上**放寬**上限。**此提議遭否決**，理由有三：

1. **與驗收標準 A-2 直接衝突。** 3 GB 不是隨手設的數字，它是專案的設計承諾；§5.4.2 推導的預期峰值為 655 MB，3 GB 已有 4.5 倍餘裕。
2. **會削弱熔斷器的偵測能力。** 熔斷器的目的是**抓 bug**（某個 chunk 失控），不是榨乾資源。若某 chunk 吃掉 3 GB，不論機器有多少 RAM 都代表出事了。在 64 GB 機器上該公式給出 6 GB，失控的 chunk 要吃到 6 GB 才被攔截——偵測變差。
3. **Chunked 架構吃不下更多記憶體。** chunk 為序列處理且有界，沒有東西可以擴張進去。

反向的取小值則有價值：在 8 GB 機器上得到 2 GB 上限，保護使用者的機器。故採**保護型**公式——只下調，不上調。
...
**典型 $\varphi$ 值（v1.6 更新為實測值）**

| 內容類型 | $\varphi$ | 來源 |
| :---- | :---- | :---- |
| **PowerShell 終端機（1728×812）** | **0.0172** | 實測 |
| **IDE 深色主題（1920×1020）** | **0.0108** | 實測 |
| **手機社群 App（1080×2400）** | **0.0054** | 實測 |
| 瀏覽器內文 16px CSS | 0.0194 | 推估，未實測 |
| 簡報內文 | 0.0320 | 推估，未實測 |
| 簡報標題 | 0.0560 | 推估，未實測 |

實測方法：原生解析度、`MSER(min_area=15)`、取通過長寬比與高度篩選之元件的高度中位數除以畫面高度。

> **v1.5 的假設值 $\varphi(\text{IDE}) = 0.0176$ 樂觀了 63%。** 實測為 0.0108。原假設取自「14pt 字型於 1080p」的紙上推算，未考慮實際 IDE 為深色主題、多面板、高密度佈局，且視窗擷取的有效高度小於全螢幕。

#### 5.6.2.1 實測後的裁切幅度（v1.6 新增）

以實測 $\varphi$ 重算達到可讀門檻所需的裁切幅度：

| 素材 | letterbox 字高 | 達 $h_{\min}=28$ px 時可見面積 | 達 $h_{\text{comfy}}=40$ px 時可見面積 |
| :---- | :---- | :---- | :---- |
| PowerShell | 8.7 px ❌ | 26.4%（滿高裁切即可） | 18.0% |
| **IDE** | 6.2 px ❌ | **16.4%** | **8.0%** |
| 手機社群 | 10.4 px ❌ | 17.1% | 8.4% |
| *IDE（v1.5 假設值）* | *10.7 px* | *31.6%（滿高即可）* | *22.6%* |

**結論修正**：§5.6.2 原本的結論是「要讓 UI 文字可讀，必須放棄約 68% 的畫面內容」。實測顯示對真實 IDE 錄影而言：

* 達到**最低**可讀門檻需放棄 **83.6%** 的畫面
* 達到**舒適**門檻需放棄 **92.0%** 的畫面——等於一次只顯示數行程式碼

後者已不能稱為「重構」，而是變成另一種產品形態（逐段程式碼導讀）。

**產品層級的意涵（規範）**

現行架構隱含承諾「提供螢幕錄影即可產出可讀的短影音」。實測顯示此承諾對典型 IDE 錄影**在幾何上無法兌現**。

規範：`plan_reframe`（§4.8）在偵測到 $\varphi$ 低於可行門檻時，**必須**回傳 `SOURCE_TEXT_TOO_SMALL` 警告，並明確建議**來源端調整**——提高錄製時的 UI 縮放比或編輯器字級。這比任何事後重構都有效，且是唯一能真正解決問題的手段。工具不得在明知文字不可讀的情況下靜默產出成品。
...
> **v1.6 定位修正**：v1.5 曾將 LLM 判讀提升為主要路徑，理由是「像素路徑經三輪實測仍不可用」。**該前提已證實不成立**——像素路徑不可用的根因是 `MSER` 預設參數（§5.6.3.9），修正後 AUC 達 0.983。
>
> **兩條路徑現皆可行，本規格不預設主從。** 像素路徑（本節）確定性、可測試、離線可用、無 API 成本；LLM 路徑（§5.6.3.8）不需校準語料、可處理未預期的內容型態。M2 須以同一語料並列評測後再決定預設路徑。
>
...
| 訊號 | 定義 | 素材層級 AUC | 角色 |
| :---- | :---- | :---- | :---- |
| **文字區塊密度 + 共線過濾** | MSER(`min_area=15`) 元件經 §5.6.3.5 共線檢驗後計數，除以每 10 萬像素 | **0.983** | **主訊號** |
| 文字區塊密度（未過濾） | 同上但不做共線檢驗 | 0.950 | 備用 |
| 色彩稀疏度 | 5-bit 量化後最常見 16 色所佔的像素比例 | 0.705 | 輔助 |
| 軸向邊緣佔比 | Sobel 梯度方向落在水平／垂直 ±8° 內的邊緣像素比例 | 0.740 | 輔助（**v1.2 誤列為主訊號**） |

AUC = 0.5 代表無判別力。主訊號的 AUC 為 3 支 `screen` 對 20 支 `natural` 的實測值（**B 級證據**，樣本數偏少）。

**主訊號的最佳操作點**（素材層級，$k=8$ 中位數聚合）
...
> ⚠️ **v1.5 曾在此標註「不得據此逕行採用」**，理由是召回面未測量、且過濾後所有樣本落在 1.2 以下顯示可能過於激進。**該顧慮已於 v1.6 解除**——但過程本身值得記錄。

**v1.6 召回面實測（採用）**

首次測試以 `MSER` 預設 `min_area=60` 進行，得出 IDE 保留率 **0%**，據此判定共線約束不可用。**該結論是錯的**——被 §5.6.3.9 的參數問題汙染。以 `min_area=15` 重測：

| 素材 | 類別 | 原始計數 | 共線過濾後 | 保留率 |
| :---- | :---- | :---- | :---- | :---- |
| PowerShell | screen | 31.41 | 7.27 | **23%** |
| 手機社群 | screen | 22.48 | 5.08 | **23%** |
| IDE | screen | 14.25 | 2.99 | **21%** |
| DEATH STRANDING | game | 2.22 | 0.16 | 7% |
| 20 支電影（中位） | natural | 3.74 | 0.41 | 11% |
| 20 支電影（最大） | natural | 23.54 | **3.85** | — |

三支 `screen` 的保留率高度一致（21–23%），而 `natural` 僅 11%、`game` 僅 7%。過濾同時把 natural 的最大值由 23.54 壓到 3.85——先前的兩個紋理型偽陽性（墓碑陣列、控制面板燈陣）被有效抑制。

**判別力提升**：AUC 0.950 → **0.983**，特異度 0.90 → **0.95**，召回維持 1.00。

**判定：採用。** `classify_collinear_filter` 預設改為 `true`。

> **保留率不是 70%，而是 21–23%——為何仍判定有效？** v1.5 訂下的「保留率 ≥ 70%」前置條件，前提是把共線過濾視為「保留真文字、濾掉紋理」的硬分類器。實測顯示它其實是**軟判別器**：兩類都被壓低，但 `screen` 被壓得少。判別力來自兩者的**比值差異**（23% vs 11%），而非絕對保留率。故該前置條件的設定本身不恰當，以 AUC 與平衡準確率取代之。
...
> **規範一：合成樣本不得作為規格中任何門檻或訊號排序的依據。**
>
> **規範二（v1.6 新增）：任何以第三方函式庫為基礎的量測，必須先驗證其預設參數對本專案的輸入型態是否適用，並在規格中顯式記錄所用參數值。** 依據見 §5.6.3.9——單一預設參數曾使四輪真實語料實測全部得出錯誤結論。
>
...
| 項目 | 數值 | 等級 | 備註 |
| :---- | :---- | :---- | :---- |
| 主訊號 = 文字區塊密度 + 共線過濾 | AUC 0.983 | **B** | 3 支 `screen` vs 20 支 `natural`。screen 樣本數偏少 |
| `mser_min_area = 15` | 元件數 2.7–3.3 倍 | **A** | 3 種素材型態、原生與代理兩種解析度一致 |
| $\varphi$ 實測值 | IDE 0.0108 等 | **B** | 每種型態各 1 支 |
| 軸向邊緣佔比應降級 | — | **A** | 兩份語料一致支持；電影 natural 中位 0.383 與 screen 完全重疊 |
| 時間靜止率應移除 | AUC 0.409 | **B** | 單一來源，但方向錯誤明確 |
| 門檻 4.9 的特異度 | 0.80 | **A** | 20 支獨立來源 |
| 門檻 1.26 的召回率 | 1.00 | **B** | 3 支 `screen`，樣本數偏少 |
| 黑邊對訊號的影響 | ≤ 0.058 | **A** | 11 支含黑邊素材的剝除前後對照 |
| 共線群組約束：偽陽性面 | 特異度 0.95 | **A** | 20 支獨立來源 |
| 共線群組約束：召回面 | 保留率 21–23% | **B** | 3 支 `screen`，保留率高度一致 |
| LLM 分類可靠度 | — | **未測量** | 仍未在任何語料上評測（R-14） |
...
##### 5.6.3.8 LLM 視覺分類（v1.5 新增，v1.6 改為並列路徑）

**v1.6 定位修正**：v1.5 引入本路徑的理由是「像素分類器不可用」。該理由已失效（§5.6.3.9）。本路徑仍具獨立價值，但**不再是唯一可行路徑**。

**保留的理由**

| 優勢 | 說明 |
| :---- | :---- |
| 不需校準語料 | 像素路徑的門檻仍建立在 3 支 `screen` 上（B 級證據） |
| 可處理未預期型態 | 像素訊號只在已見過的內容型態上驗證過 |
| 邊際成本近乎為零 | 縮圖本來就會送給 LLM 以產生 EDL |
| 可解釋 | `reasoning` 欄位使誤判可追溯 |

**像素路徑的相對優勢**：確定性、可寫單元測試、離線可用、無 API 延遲與成本、不受模型版本變動影響。

**成本近乎為零**：LLM 本來就在管線中，§4.3 的縮圖本來就會送給它以產生 EDL。分類只是在既有的結構化輸出中多加幾個欄位，不增加任何影像傳輸。

**輸入**：§4.3 產出的 LLM 判讀縮圖，每支素材拼貼為 1 張（§4.9.6）。拼貼格內須標註索引與時間碼，供模型指涉。

**輸出**（併入既有結構化輸出，逐素材）

```json
{
  "source_file": "screen_rec_01.mp4",
  "content_class": "screen_recording",
  "confidence": "high",
  "reasoning": "畫面為程式碼編輯器，含側欄檔案樹與下方終端機面板",
  "focus_region": "center",
  "has_readable_text": true,
  "suggested_reframe_mode": "region_pan"
}
```

**規範**

1. `content_class` 的列舉值與 §5.6.3.7 一致（`natural` / `screen_recording` / `slideshow` / `mixed` / `unknown`）。
2. **必須要求模型輸出 `reasoning`**。無法解釋的分類無從除錯，且可據此察覺模型是因縮圖過小而瞎猜（§4.9.5）。
3. §5.6.3.7 的**誤判代價不對稱**規則**同樣適用**——LLM 也會錯。System Prompt 須明示：不確定時偏向 `screen_recording`。
4. 使用者的 `--content-class` 覆寫**優先於 LLM 判斷**。
5. **LLM 不得用於估計 $\varphi$**（文字高度佔比）。模型無法可靠回報「這段文字有 19 像素高」，$\varphi$ 必須由像素量測取得（見附錄 A-10）。LLM 只回報布林的 `has_readable_text`。

**後備關係**

| 情境 | 使用路徑 |
| :---- | :---- |
| LLM 可用（預設） | LLM 判讀；像素訊號同時計算並記錄，供日後比對 |
| 無 LLM（離線模式） | 像素路徑（§5.6.3.1–§5.6.3.7），且門檻未校準時拒絕自動分類 |
| 兩者判斷不一致 | 採 LLM 結果，但於輸出中標示 `classifier_disagreement: true` |

**驗證要求**：LLM 分類的可靠度**尚未量測**。取得 §10 M1 的 `screen` 類語料後，須與像素路徑在同一批語料上並列評測。**不得因其方便而免除驗證**——這正是 §5.6.3.6 證據分級所要防範的。目前 LLM 路徑的證據等級為 **未測量**。

##### 5.6.3.9 根因分析：MSER 預設參數（v1.6 新增）

**問題**：`cv2.MSER_create()` 的 `min_area` 預設值為 **60**（像素面積）。

本專案在 1280 px 代理解析度上分析素材。以 IDE 深色主題錄影為例，原生 1920×1020 縮至 1280×680（0.667 倍），原始約 14 px 高的程式碼文字變為約 9 px；單一字元寬約 5 px，面積約 **45 px²**——**低於 MSER 的 `min_area` 門檻，該區域根本不會被回傳**。

**實測影響**

| 素材 | 解析度 | `min_area=60` | `min_area=30` | `min_area=15` | `min_area=8` |
| :---- | :---- | :---- | :---- | :---- | :---- |
| IDE | 1920×1020（原生） | 63 | 151 | **210** | 225 |
| IDE | 1280×680（代理） | 42 | 75 | **138** | 166 |
| PowerShell | 1728×812（原生） | 75 | 153 | **206** | 233 |
| 手機社群 | 1080×2400（原生） | 349 | 521 | **679** | 740 |

`min_area=15` 相對預設值多偵測 **2.7–3.3 倍**的元件，且 `min_area=8` 的增益已趨緩——顯示 15 已接近本專案輸入型態的合適值。

**規範**：`[reframe] mser_min_area = 15`。此參數**必須顯式設定**，不得依賴函式庫預設值。

**此參數汙染過的結論**（全部已於 v1.6 修正）

| 版本 | 錯誤結論 | 真實情況 |
| :---- | :---- | :---- |
| v1.3 | 文字區塊密度 AUC 0.929（單來源自助估計） | 0.983（含共線過濾） |
| v1.4 | 門檻 4.9，特異度 0.80 | 門檻 1.26，特異度 0.95 |
| v1.5 | 共線群組召回面「無法測量」，疑似過於激進 | 保留率 21–23%，有效 |
| v1.5 | 合成文字測試各字級皆回傳 0，判定為「合成影像假象」 | 同一參數問題，該判定亦錯誤 |
| v1.5 | $\varphi(\text{IDE})$ 假設 0.0176 | 實測 0.0108 |

**教訓（納入 §5.6.3.6 規範）**

> **任何以第三方函式庫為基礎的量測，必須先驗證該函式庫的預設參數對本專案的輸入型態是否適用。**

此教訓與 v1.4 的「合成樣本不可作為門檻依據」不同且更基本：本次每一輪實測都用了真實語料、內部邏輯自洽、結論看似穩固——但四輪全錯，因為量測工具本身在預設組態下對目標訊號是盲的。**真實語料不足以保證結論正確，量測儀器也必須先校準。**
...
memory_limit_mb        = 0         # 0 = 自動：min(3072, 0.25 * 系統總RAM)，見 §5.4.3
                                   # 明確指定正整數則覆寫自動計算
temp_dir               = ""        # 留空則用系統暫存目錄
keep_temp              = false

[llm]                                  # §4.9 視覺輸入
thumb_width             = 858          # LLM 判讀縮圖寬度（§4.9.5）
contact_sheet           = true         # 每支素材拼貼為 1 張（§4.9.4 圖片數上限）
contact_sheet_grid      = "3x3"        # 最多 9 幀
classify_by_llm         = true         # §5.6.3.8 LLM 分類為主要路徑
count_tokens_on_start   = true         # 啟動時以供應商端點實測視覺 token 用量（§4.9.6）

[sampling]                             # §4.3 抽樣策略
scene_detect            = true         # 以 PySceneDetect 分鏡後抽代表幀
scene_detect_threshold  = 27.0         # ContentDetector 門檻
scene_detect_on_screen  = false        # screen_recording 類停用（捲動非切換）
proxy_width             = 360          # 像素分析代理寬度，維持不變
...
[beat]                                 # §5.2 節拍吸附
analysis_sr             = 44100        # §4.2.1 必須顯式指定，勿用 librosa 預設的 22050
hop_length              = 512          # 必須同時傳給 onset_strength 與 beat_track（§5.2.1）
#   時間解析度 = hop_length/analysis_sr = 11.6ms，佔 A-3 預算(50ms) 23%
#   若 A-3 實測餘裕不足，可降至 256（5.8ms），代價是包絡線長度加倍
bpm_from_mean_interval  = true         # §4.2.1 不得採用 beat_track 的 tempo 標量
onset_weight_gamma      = 0.3          # beta = gamma * (60/BPM)，範圍 0.2~0.4；0 = 停用能量加權
onset_norm_low_pct      = 10           # 穩健正規化的下百分位
onset_norm_high_pct     = 90
...
# --- 分類器（§5.6.3）---
# 主訊號：文字區塊密度 + 共線過濾（§5.6.3.5），v1.6 以真實 screen 語料校準。
classify_aggregate_frames        = 8      # 素材層級聚合幀數（§5.6.3.2），沿用 sample_max_frames
classify_text_density_threshold  = 1.26   # 主訊號（共線過濾後），v1.6 實測暫定值
#   3 支 screen vs 20 支 natural：召回 1.00 / 特異度 0.95 / 平衡準確率 0.97
#   樣本數偏少（B 級證據），語料擴充後須重新校準。
#   未啟用共線過濾時的對應門檻為 8.26。
classify_collinear_filter        = true   # 共線群組約束（§5.6.3.5），v1.6 實測後啟用
#   AUC 0.983 / 召回 1.00 / 特異度 0.95（3 支 screen vs 20 支 natural，B 級證據）
mser_min_area                    = 15     # §5.6.3.9 必須顯式設定，不得用函式庫預設 60
...
│   │   ├── llm_classifier.py    # §5.6.3.8 LLM 視覺分類（主要路徑）
│   │   ├── contact_sheet.py     # §4.9.6 拼貼縮圖組裝
│   │   ├── scene_sampler.py     # §4.3 PySceneDetect 自適應抽樣
...
│   │
│   ├── capcut/
│   │   ├── adapter.py           # §4.4.1 適配器抽象介面
│   │   ├── backend_builtin.py   # 內建草稿產生器
│   │   ├── backend_community.py # 社群函式庫後端（授權釐清後才啟用）
...
│
├── scripts/
│   ├── check_env.py             # 環境自檢（FFmpeg / 編碼器 / Python 版本 / MSER 參數）
│   ├── spike_filtergraph.py     # Spike 1：濾鏡鏈與管線回歸驗證（§5.3 / §5.4 / §5.5）
...
| T-16 | 節拍吸附的統計顯著性 | 涉及隨機取樣的效果量測（如 §5.2.1 能量加權）**必須至少 300 次重抽**並回報標準差。實測顯示 n=25 時標準差約 10%，單次抽樣的方向本身即為雜訊 |
...
| **R-05** | **CapCut 草稿格式失效**（v1.1 新增） | **高** | `draft_content.json` 為未公開的逆向工程格式，無相容性承諾；應用程式更新可隨時破壞產生器。 | ① 在 `capcut/schema/` 保存已驗證版本的結構樣板並註明實測版本號；② 寫入前比對目標軟體版本，不符時回傳 `CAPCUT_SCHEMA_UNSUPPORTED` 警告而非產出損毀草稿；③ **軌道 B 為獨立且不受此影響的替代路徑**——這正是雙軌架構的核心價值；④ 產出前自動備份既有草稿目錄；⑤ **v1.5 新增**：以 §4.4.1 適配器層隔離後端，使 EDL→草稿的映射邏輯與具體格式實作解耦，後端失效時切換成本侷限於單一模組。 |
...
| **R-13** | **校準語料不足**（v1.2 新增，v1.4 更新） | **高** | §5.6.3 的門檻仍未以多來源語料校準。**此風險已連續兩度實現**：① v1.2 依合成樣本推定的主訊號與門檻，經單一真實素材驗證即遭推翻（準確率 0.49，低於基準線 0.70）；② v1.3 由單一來源估計的特異度 0.95，經 20 支獨立來源實測實為 **0.80**，偽陽性率被低估 4 倍。**每次擴充語料都推翻上一版結論。** | ① **所有分類門檻一律設為 `-1`（未校準）**，包含主訊號——4.9 已因特異度不足而撤回；② §5.6.3.6 訂立證據品質分級，並禁止以合成樣本作為門檻依據；③ **實作在主訊號未校準時必須拒絕啟動自動分類**，改要求使用者顯式指定素材類型，而非用猜測值靜默運行；④ §5.6.3.7 已把自動分類降級為建議值，CLI 須提供 `--content-class` 覆寫，降低對分類器的依賴；⑤ 語料收集與門檻校準列為 **M1 退出條件**；⑥ `scripts/calibrate_classifier.py` 自動化校準流程，語料擴充後可重跑；⑦ v1.5 緩解：引入 §5.6.3.8 的 LLM 判讀路徑，不需校準語料；⑧ **v1.6 大幅緩解**：取得 `screen` 語料並修正 §5.6.3.9 的參數問題後，像素路徑已可用（AUC 0.983、召回 1.00、特異度 0.95），門檻由 `-1` 改為 1.26。**本風險已從「高」實質降為「中」**，但 `screen` 樣本僅 3 支，門檻仍為暫定值，故不予降級標示，待語料擴充後重評。 |
| **R-14** | **LLM 分類可靠度未經量測**（v1.5 新增，v1.6 降低影響） | 低 | §5.6.3.8 將分類職責移交 LLM，繞過了語料阻擋，但 LLM 分類的準確率**尚未在任何語料上量測過**。方便性不等於正確性——這正是 §5.6.3.6 證據分級所要防範的模式。此外，§4.9.5 已證實 360p 縮圖下模型看不見 UI 文字，若縮圖尺寸設定錯誤，模型會在無足夠資訊的情況下猜測。 | ① 強制要求模型輸出 `reasoning`，使「因看不清而瞎猜」可被察覺；② LLM 判斷與像素路徑同時計算並比對，不一致時標示 `classifier_disagreement`；③ 使用者 `--content-class` 覆寫優先於 LLM；④ 取得 `screen` 類語料後，**必須**與像素路徑並列評測，不得因方便而免除驗證；⑤ 縮圖尺寸由 §4.9.6 規範並於啟動時以 token 計數端點驗證；⑥ **v1.6**：像素路徑已可用，LLM 不再是唯一路徑，本風險的影響由「中」降為「低」——LLM 若不可靠，可退回像素路徑而非失去分類能力。 |
| **R-16** | **來源文字過小，幾何上無法補救**（v1.6 新增） | **高** | 實測顯示真實 IDE 錄影（$\varphi = 0.0108$）即使採最激進的裁切，也只能在顯示 16.4% 畫面時勉強達到最低可讀門檻；舒適門檻需裁到 8.0%。手機直式素材的縮放比為 1.0，文字完全不會放大。**這是幾何限制，非演算法可解。** | ① `plan_reframe` 偵測到 $\varphi$ 過低時**必須**回傳 `SOURCE_TEXT_TOO_SMALL` 警告；② 警告中明確建議**來源端調整**（提高錄製時的 UI 縮放比或編輯器字級）——這是唯一有效的解法；③ 不得在明知文字不可讀的情況下靜默產出成品；④ README 與使用者指引須說明此限制，避免產品承諾超出可兌現範圍。 |
| **R-15** | **無 LLM 時的離線降級**（v1.5 新增） | 低 | 主要分類路徑依賴 LLM，離線或 API 失敗時無法分類。 | 像素路徑（§5.6.3.1–§5.6.3.7）保留為完整後備；門檻未校準時拒絕自動分類並要求使用者以 `--content-class` 指定，而非猜測。 |
...
- [x] **安裝 FFmpeg**（實測 9.0.1 full build）並完成 §3.2.2 編碼器冒煙測試——選出 `h264_nvenc`，`h264_amf` 雖編入建置但實測失敗並被正確拒絕。
- [x] 撰寫 `scripts/check_env.py` 環境自檢腳本。
- [x] 撰寫 `scripts/make_fixtures.py`，產出 7 項合成測試素材（含燒入影格號的精確性驗證素材）。
- [x] **Spike 1：首次實際執行 §5.3 / §5.5 / §5.6.8 濾鏡鏈**，7/7 通過；發現記憶體模型低估 1.85 倍（§5.4.4）。
- [x] **Spike 2：librosa 節拍分析實測**。R-10 解除（librosa 1.0.0 + numba 0.67.0 + Python 3.12）；發現並修正取樣率與 BPM 計算法兩項缺陷（§4.2.1）；7/7 通過。
- [ ] 封裝為正式模組，輸出精確時間戳 JSON；實作 `infer_4_4` 重拍推定（§4.2）。
- [ ] 封裝 `yt-dlp` 下載與轉碼流程，含 §11.3 合規提醒。
- [x] 建立具備 9:16 自適應與動態模糊之獨立片段裁切腳本（§5.3、§5.5）——見 `scripts/spike_filtergraph.py`。
- [ ] 以真實攝影素材複驗 §5.3.1 模糊最佳化的畫質（合成素材不可拍板，見 §5.6.3.6）。
- [x] T-04（影格精確切片）通過：90/90 幀，誤差 0。
- [x] T-06（混合幀率拼接）通過：漂移 0.0 ms。
- [x] T-01（節拍偵測）通過：BPM 120.00，間隔標準差 0.0029 s。
- [x] T-02（吸附不漂移）通過：終點誤差 0.00 s。
- [ ] T-05（旋轉旗標）測試案例。
...
- [x] 取得 `screen` 類語料（PowerShell / IDE / 手機社群 / 遊戲，共 4 支），首次量測召回面。
- [x] 找出並修正 §5.6.3.9 的 `MSER min_area` 根因；分類器 AUC 0.983、召回 1.00、特異度 0.95。
- [x] 實測 $\varphi$，修正 §5.6.2 的可讀性模型假設。
- [ ] **擴充 `screen` 語料至 10 支以上**（目前 3 支，門檻 1.26 為 B 級證據）。涵蓋淺色 IDE 主題、簡報錄製、中文軟體介面。
...
- [ ] 實作 §4.3 雙解析度抽樣與 PySceneDetect 分鏡抽樣（`auto_downscale` 必須啟用）。
- [ ] 以供應商 token 計數端點實測 §4.9 的視覺輸入用量，驗證核算模型。

**M1 退出條件**：三個核心模組皆可獨立以 CLI 執行；§5.3 與 §5.6.8 全部濾鏡鏈經實機驗證（**v1.7 已完成**）；分類器門檻已用真實語料校準（**v1.6 已完成，門檻 1.26**）；T-01、T-04、T-05、T-11、T-12 通過。

**M1 目前進度**：濾鏡鏈驗證（v1.7）、分類器校準（v1.6）、音訊分析（v1.8）三項核心驗證均已完成，**M1 的技術阻擋項已全部清除**。剩餘為工程化工作：將 spike 腳本封裝為正式模組、補齊 T-05 與 T-11／T-12 測試案例、以真實攝影素材複驗 §5.3.1 模糊畫質。
...
- [ ] 實作 §5.6.3.8 LLM 視覺分類與 §4.9.6 拼貼縮圖組裝。
- [ ] **並列評測像素路徑與 LLM 路徑**（§5.6.3 頂部規範），以同一語料決定預設路徑。
- [ ] 實作 §5.2.1 起音能量加權吸附，含穩健正規化與 `hop_length` 一致性檢查。
...
- [ ] 實作 §4.4.1 適配器抽象介面與內建產生器後端。
- [ ] 依 §4.4.2 決定社群函式庫後端：確認 `pyCapCut` 授權狀態，或改以 `pyJianYingDraft`（Apache-2.0）支援剪映路徑。
...
| **A-2** | **CapCut / 剪映目標版本**：~~兩者格式是否有差異？~~ **v1.5 已部分解答**——同一作者維護 `pyCapCut` 與 `pyJianYingDraft` 兩套分離的程式碼庫，證實兩者格式不同，**必須分別支援**（§4.4.2）。**仍待決定**：v1.0 要支援哪一個，或兩者都支援？以及目標軟體版本號。 | 直接影響 R-05 風險與 A-5 驗收 | M3-A 開始前 |
...
| **A-8** | **校準語料規模與涵蓋面**：已完成兩輪——1 支科技評論影片（推翻 v1.2 訊號排序）與 20 支電影片段（推翻 v1.3 特異度估計）。**v1.6 更新**：`screen` 類已取得 4 支（PowerShell / IDE / 手機社群 / 遊戲），足以完成首輪召回量測並找出 §5.6.3.9 的根因。**但 3 支有效樣本仍偏少**，門檻 1.26 為 B 級證據。仍需擴充至 10 支以上，優先補齊：淺色 IDE 主題、簡報錄製、中文軟體介面。**特別需要目標情境的素材**——首輪語料的 `screen` 樣本多為紀錄片式截圖插入，而非使用者自錄的完整螢幕錄影。建議涵蓋：深色／淺色 IDE 主題、不同 UI 縮放比、中文／英文介面、終端機、簡報錄製。 | 直接決定 R-13 的殘餘風險 | M1 開始前 |
...
| **A-10** | **文字高度估計（φ）的取得方式**：目前以 MSER 連通元件估計，而 §5.6.3.5 已證實 MSER 無法區分文字與紋理。**v1.5 重新定位**：LLM 路徑（§5.6.3.8）解決了*分類*，但**無法估計 φ**——模型無法可靠回報像素字高。故 φ 仍需像素量測，這才是輕量文字檢測模型（DBNet / PaddleOCR-det ONNX）的真正用武之地，而非分類。**但成本待實測**：外部建議宣稱「延遲 10–20 ms」，查證後 PP-OCRv5 mobile 的 CPU ONNX 實測為 **688 ms @ 2048 解析度**（差 34–69 倍）。以 1280 代理解析度概估仍需約 268 ms/幀，10 支素材 × 8 幀約 21 秒，佔驗收 A-6 預算（≤ 90 秒）的 23%。**採用前必須實測。** | 影響可讀性預算的可信度 | M1 校準時一併評估 |
