...
| 版本號類型 | 目前值 | 定義 | 遞增時機 |
| :---- | :---- | :---- | :---- |
| **文件版本 (Document Version)** | **v1.5** | 本規格書自身的修訂版次。描述「我們打算做什麼」。 | 規格內容有實質變更時遞增（如新增工具、修改 Schema、調整架構）。 |
...
| **v1.5** | **2026-09-04** | **外部架構建議評估與查證後採納**。見下方變更明細。 |
...
> **v1.5 架構變更**：像素級分類經 v1.2–v1.4 三輪實測，仍無法定出可用門檻（`screen` 類零樣本，所有門檻為 `-1`，自動分類無法啟動）。v1.5 起改以 **LLM 視覺判讀為主要路徑**（§5.6.3.8），本節其餘內容降為**離線後備路徑**——當無 LLM 可用，或需與 LLM 判斷交叉驗證時使用。門檻未校準的限制僅適用於後備路徑。
>
...
##### 5.6.3.8 LLM 視覺分類（v1.5 新增，主要路徑）

**動機**：像素級分類器歷經三個版本仍不可用，根因是缺少 `screen` 類語料，而取得語料需要使用者投入。LLM 判讀繞過整個困局——它**不需要校準語料**。
...
| **R-13** | **校準語料不足**（v1.2 新增，v1.4 更新） | **高** | §5.6.3 的門檻仍未以多來源語料校準。**此風險已連續兩度實現**：① v1.2 依合成樣本推定的主訊號與門檻，經單一真實素材驗證即遭推翻（準確率 0.49，低於基準線 0.70）；② v1.3 由單一來源估計的特異度 0.95，經 20 支獨立來源實測實為 **0.80**，偽陽性率被低估 4 倍。**每次擴充語料都推翻上一版結論。** | ① **所有分類門檻一律設為 `-1`（未校準）**，包含主訊號——4.9 已因特異度不足而撤回；② §5.6.3.6 訂立證據品質分級，並禁止以合成樣本作為門檻依據；③ **實作在主訊號未校準時必須拒絕啟動自動分類**，改要求使用者顯式指定素材類型，而非用猜測值靜默運行；④ §5.6.3.7 已把自動分類降級為建議值，CLI 須提供 `--content-class` 覆寫，降低對分類器的依賴；⑤ 語料收集與門檻校準列為 **M1 退出條件**；⑥ `scripts/calibrate_classifier.py` 自動化校準流程，語料擴充後可重跑；⑦ **v1.5 根本性緩解**：改以 §5.6.3.8 的 LLM 視覺判讀為主要分類路徑。該路徑**不需要校準語料**，使本風險對 v1.0 交付不再構成阻擋；像素路徑降為離線後備，其未校準狀態不影響主流程。**但代價是引入 R-14。** |
| **R-14** | **LLM 分類可靠度未經量測**（v1.5 新增） | 中 | §5.6.3.8 將分類職責移交 LLM，繞過了語料阻擋，但 LLM 分類的準確率**尚未在任何語料上量測過**。方便性不等於正確性——這正是 §5.6.3.6 證據分級所要防範的模式。此外，§4.9.5 已證實 360p 縮圖下模型看不見 UI 文字，若縮圖尺寸設定錯誤，模型會在無足夠資訊的情況下猜測。 | ① 強制要求模型輸出 `reasoning`，使「因看不清而瞎猜」可被察覺；② LLM 判斷與像素路徑同時計算並比對，不一致時標示 `classifier_disagreement`；③ 使用者 `--content-class` 覆寫優先於 LLM；④ 取得 `screen` 類語料後，**必須**與像素路徑並列評測，不得因方便而免除驗證；⑤ 縮圖尺寸由 §4.9.6 規範並於啟動時以 token 計數端點驗證。 |
...
- [ ] 實作 §5.6.3.8 LLM 視覺分類（主要路徑）與 §4.9.6 拼貼縮圖組裝。
