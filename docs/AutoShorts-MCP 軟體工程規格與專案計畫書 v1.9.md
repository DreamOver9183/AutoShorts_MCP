...
| 版本號類型 | 目前值 | 定義 | 遞增時機 |
| :---- | :---- | :---- | :---- |
| **文件版本 (Document Version)** | **v1.9** | 本規格書自身的修訂版次。描述「我們打算做什麼」。 | 規格內容有實質變更時遞增（如新增工具、修改 Schema、調整架構）。 |
...
| **v0.2** ✅ | M1 — 影音核心模組 | **已完成（2026-09-04）**。節拍分析、媒體探測、9:16 裁切模組化並通過 52 項測試。音訊下載器（Tool 1）移交 M2。 |
...
| v1.8 | 2026-09-04 | Spike 2：音訊分析實測，R-10 解除。 |
| **v1.9** | **2026-09-04** | **M1 收尾。封裝為正式模組時，單元測試逼出兩項演算法缺陷；修正後分類器 AUC 由 0.983 提升至 1.000。專案版本進入 v0.2。** 見下方變更明細。 |
...
將三支 spike 腳本封裝為 §7 定義的模組結構並補齊單元測試。**寫測試的過程逼出兩項演算法缺陷**——這兩項在 spike 階段都沒暴露，因為 spike 只跑真實素材，而真實素材恰好落在缺陷不顯現的區間。

**缺陷一：MSER 巢狀區域未去重（§5.6.3.5）**

MSER 對同一個字元會回傳多個嵌套的穩定區域。共線分組把這些嵌套框當成不同元件，導致列內出現大量負間距。

修正（加入 IoU 去重）後判別力顯著提升：

| 指標 | 去重前 | **去重後** |
| :---- | :---- | :---- |
| AUC | 0.983 | **1.000** |
| `natural` 最大值 | 3.85 | **0.16** |
| `screen` 最小值 | 2.99 | 2.18 |
| 特異度 | 0.95 | **1.00** |

兩類之間現在有一整個數量級的空隙（natural ≤ 0.16、screen ≥ 2.18）。門檻由 1.26 改為 **2.96**。

**缺陷二：間距檢查隱含假設 MSER 的粒度（§5.6.3.5）**

原設計以「字高」為尺度限制列內水平間距（`gap_max = 2.5 × 字高`）。此尺度**隱含假設 MSER 回傳字元級元件**。

實測發現 MSER 在乾淨的抗鋸齒文字上會回傳**詞級**區域，此時間距（59 / 23 / 113 px）相對於 `2.5 × 12 = 30` 過大，整列被誤拒、保留率歸零。真實素材上 MSER 回傳字元級元件（IDE 有 210 個），間距小而僥倖通過——**代表原本 0.983 的表現依賴 MSER 粒度，而粒度並不穩定**。

以真實語料比較三種修法：

| 變體 | AUC | 門檻 | 召回 | 特異度 |
| :---- | :---- | :---- | :---- | :---- |
| 未過濾 | 0.983 | 9.94 | 1.00 | 0.95 |
| A：字高尺度（原設計） | 1.000 | 0.17 | 1.00 | 1.00 |
| B：改以元件寬度為尺度 | 1.000 | 0.17 | 1.00 | 1.00 |
| **C：完全移除間距檢查（採用）** | **1.000** | **2.96** | **1.00** | **1.00** |

三者判別力相同——**間距檢查不貢獻任何判別力**，工作全由「去重 + 共線 + 高度一致性」完成。採用 C：少一個參數，且不再依賴 MSER 粒度。

> **教訓**：spike 只跑真實素材時，「碰巧可用」與「設計正確」無法區分。單元測試以合成輸入探測邊界條件，正是為了分辨這兩者。這與 §5.6.3.6「合成樣本不得作為門檻依據」不衝突——**合成樣本不能定門檻，但可以暴露演算法的隱含假設**。已將此區分補入 §5.6.3.6。

**缺陷三：FFmpeg 9 的旋轉中繼資料寫法變更（T-05）**

舊式 `-metadata:s:v:0 rotate=90` 在 FFmpeg 9 上**不再寫入** rotation side data，須改用輸入側的 `-display_rotation`。T-05 測試案例據此更新，現已實際執行（先前因無法產生素材而跳過）。

**M1 完成，專案版本進入 v0.2**

| M1 退出條件 | 狀態 |
| :---- | :---- |
| 三個核心模組可獨立執行 | ✅ 封裝為 `src/autoshorts_mcp/` 下的模組 |
| §5.3 / §5.6.8 濾鏡鏈經實機驗證 | ✅ v1.7 Spike 1，7/7 |
| 分類器門檻已用真實語料校準 | ✅ 門檻 2.96，AUC 1.000 |
| T-01 / T-02 / T-04 / T-05 / T-06 / T-11 / T-12 | ✅ 全數通過 |
| 測試總數 | **52 通過，0 跳過** |

**尚未完成、移交後續里程碑的項目**

* 以真實攝影素材複驗 §5.3.1 模糊最佳化的畫質（合成素材不可拍板）。
* `screen` 語料擴充至 10 支以上（目前 3 支，門檻仍為 B 級證據）。
* `audio/downloader.py`（Tool 1）尚未實作——Spike 2 直接使用本機 WAV。
...
>
> **但合成樣本有一項正當用途（v1.9 補充）：暴露演算法的隱含假設。** 真實素材可能全部落在缺陷不顯現的區間，此時「碰巧可用」與「設計正確」無法區分。v1.9 的兩項缺陷（MSER 巢狀區域、間距檢查依賴粒度）都是以合成輸入探測邊界條件才發現的。**定門檻用真實語料，找 bug 用合成邊界案例**——兩者不衝突。
>
...
classify_text_density_threshold  = 2.96   # 主訊號（共線過濾 + 去重），v1.9 實測
#   3 支 screen vs 20 支 natural：AUC 1.000 / 召回 1.00 / 特異度 1.00
#   實測分布：screen 2.18~4.54、game 0.00、natural 中位 0.00 最大 0.16
#   樣本數偏少（B 級證據），語料擴充後須重新校準。
#   未啟用共線過濾時的對應門檻為 9.94。
classify_dedupe_iou              = 0.5    # §5.6.3.5 MSER 巢狀區域去重門檻
...
#   v1.9 移除 classify_collinear_gap_max：該檢查以字高為尺度，隱含假設 MSER
#   回傳字元級元件，且實測不貢獻判別力（移除後 AUC 仍為 1.000）。
...
- [x] 封裝為正式模組（`audio/beats.py`），輸出精確時間戳；實作 `infer_4_4` 重拍推定（§4.2）。
- [ ] 封裝 `yt-dlp` 下載與轉碼流程，含 §11.3 合規提醒。**（移交 M2）**
...
- [x] T-05（旋轉旗標）通過。實測發現 FFmpeg 9 已不支援 `-metadata rotate=90`，須改用 `-display_rotation`。
...
- [x] 實作 §5.6.2 可讀性預算模型（`preprocess/legibility.py`）；T-11、T-12 通過。
- [ ] §5.6.4 活動熱區偵測（`preprocess/activity.py`）**（移交 M2，分段裁切需要）**。
...
**M1 已完成（v1.9，專案版本 v0.2）**：濾鏡鏈驗證（v1.7）、分類器校準（v1.6）、音訊分析（v1.8）、模組封裝與測試（v1.9，52 項全通過）。

移交後續里程碑：① `audio/downloader.py`（Tool 1）；② 以真實攝影素材複驗 §5.3.1 模糊畫質；③ `screen` 語料擴充至 10 支以上。
